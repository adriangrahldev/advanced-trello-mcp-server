...
exports.registerCardsTools = registerCardsTools;
const zod_1 = require("zod");
/**
 * Register all Cards API tools
 * Based on https://developer.atlassian.com/cloud/trello/rest/api-group-cards/
 */
function registerCardsTools(server, credentials) {
    // POST /cards - Create a new card
    server.tool('create-card', {
        name: zod_1.z.string().describe('Name of the card'),
        description: zod_1.z.string().optional().describe('Description of the card'),
        listId: zod_1.z.string().describe('ID of the list to create the card in'),
    }, async ({ name, description, listId }) => {
        try {
            const response = await fetch(`https://api.trello.com/1/cards?key=${credentials.apiKey}&token=${credentials.apiToken}`, {
                method: 'POST',
                headers: {
    ...
    server.tool('move-card', {
        ...
        try {
            ...
            const response = await fetch(`https://api.trello.com/1/cards/${cardId}?key=${credentials.apiKey}&token=${credentials.apiToken}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    server.tool('move-cards', {
        ...
        try {
            ...
            const results = await Promise.all(cards.map(async (card) => {
                const response = await fetch(`https://api.trello.com/1/cards/${card.cardId}?key=${credentials.apiKey}&token=${credentials.apiToken}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    server.tool('add-comment', {
        ...
        try {
            ...
            const response = await fetch(`https://api.trello.com/1/cards/${cardId}/actions/comments?key=${credentials.apiKey}&token=${credentials.apiToken}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    server.tool('add-comments', {
        ...
        try {
            ...
            const results = await Promise.all(comments.map(async (comment) => {
                const response = await fetch(`https://api.trello.com/1/cards/${comment.cardId}/actions/comments?key=${credentials.apiKey}&token=${credentials.apiToken}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
